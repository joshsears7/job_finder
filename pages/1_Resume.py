...
if not profile:
    ...
    st.markdown(
        "<div class='page-sub'>Upload your resume to get an instant score, section breakdown, bullet coaching, and ATS keyword analysis.</div>",
        unsafe_allow_html=True,
    )

    uploaded = st.file_uploader(
        "Upload your resume (PDF or DOCX)", type=["pdf", "docx"], label_visibility="visible"
    )
    if uploaded:
        _MAX = 10 * 1024 * 1024
        ...
        try:
            ...
            with st.spinner("Analyzing your resume — loading AI model on first run (30–60 s)…"):
                ...
                if not text or len(text.strip()) < 50:
                    st.error(
                        "Could not extract text from this file. Try a different PDF or paste your resume as text."
                    )
                    st.stop()
                p = parse_resume(text)
                get_model()
...
with tab_edit:
    ...
    edited_text = st.text_area(
        "Resume Text", value=profile["raw_text"], height=900, label_visibility="collapsed"
    )
    ...
    with col_update:
        if st.button("Update Analysis", type="primary"):
            ...
            for k in ["dashboard_jobs", "cover_letter", "ats_result", "interview_qs"]:
                st.session_state.pop(k, None)
            st.success(
                f"Updated! New score: {new_analysis['overall_score']}% (Grade {new_analysis['grade']})"
            )
    ...
    with _vault_col1:
        _ver_name = st.text_input(
            "Version name",
            placeholder="e.g. Tech v1, Finance focus, Short version",
            key="vault_name_input",
        )
        if st.button("Save Current as Version", key="vault_save_btn") and _ver_name.strip():
            ...
            st.success(
                f"Saved '{_ver_name.strip()}' to vault. Jobs saved from now will be tagged with this version."
            )
            st.rerun()
    ...
    if vault:
        with st.expander(f"Vault — {len(vault)} saved version(s)", expanded=False):
            ...
            if len(_scored_versions) >= 2:
                ...
                st.markdown(
                    "<div style='margin-top:12px;font-size:12px;font-weight:600;color:#475569'>Score Progression</div>",
                    unsafe_allow_html=True,
                )
...
with tab_analyze:
    ...
    with col_score:
        st.markdown(
            f"<div style='text-align:center;padding:16px 0'>{score_ring(overall, size=130)}</div>",
            unsafe_allow_html=True,
        )
        if dims:
            ...
            st.markdown(
                ...
                "letter-spacing:.06em;margin-bottom:10px'>Score Breakdown</div>"
    ...
    with col_breakdown:
        for sec_name, sec_data in analysis["section_analyses"].items():
            ...
            sec_grade = (
                "A" if s >= 85 else "B" if s >= 70 else "C" if s >= 55 else "D" if s >= 40 else "F"
            )
            ...
            st.markdown(
                ...
                + "".join(
                    f"<div style='font-size:12px;color:#dc2626;margin-top:3px;padding-left:2px'>⚠ {i}</div>"
                    for i in sec_data["issues"]
                )
                + "".join(
                    f"<div style='font-size:12px;color:#2563eb;margin-top:3px;padding-left:2px'>💡 {sg}</div>"
                    for sg in sec_data["suggestions"]
                )
                + "</div>",
                unsafe_allow_html=True,
            )
    ...
    if analysis["missing_sections"]:
        ...
        for sec in analysis["missing_sections"]:
            st.markdown(
                alert(f"<b>{sec}</b> — {analysis['missing_tips'].get(sec, '')}", "red"),
                unsafe_allow_html=True,
            )
...
with tab_bullets:
    ...
    if not bullets:
        st.info(
            "No bullet points detected. Make sure your Experience section is in the resume text."
        )
    else:
        strong = [b for b in bullets if b["score"] >= 70]
        weak = [b for b in bullets if b["score"] < 70]
        ...
        def _local_bullet_rewrites(bullet_text):
            ...
            rewrite3 = (
                f"{rewrite1.rstrip('.')} to achieve [outcome], saving [time/money] across [scope]."
            )
            return [
        ...
        if weak:
            ...
            for idx, b in enumerate(sorted(weak, key=lambda x: x["score"])):
                ...
                grade_l = "D" if b["score"] < 40 else "C" if b["score"] < 55 else "B-"
                ...
                with col_local:
                    if st.button(
                        "⚡ Quick Rewrites", key=f"local_b_btn_{idx}", use_container_width=True
                    ):
...
with tab_suggestions:
    ...
    if dims:
        ...
        sc1.markdown(
            _score_card("Impact", d_impact, "Measurable results & outcomes"), unsafe_allow_html=True
        )
        sc2.markdown(
            _score_card("Clarity", d_clarity, "Specific, active language"), unsafe_allow_html=True
        )
        sc3.markdown(
            _score_card("Structure", d_structure, "Sections & completeness"), unsafe_allow_html=True
        )
    ...
    if not quick_wins and not high_impact:
        st.markdown(
            alert(
                "Your resume looks solid! Use the Tailor tab to optimize it for a specific job description.",
                "green",
            ),
            unsafe_allow_html=True,
        )
...
with tab_tailor:
    st.markdown(
        "<div class='page-sub'>Paste a job description — get side-by-side rewrites of your weakest bullets, targeted to <i>this specific role</i>.</div>",
        unsafe_allow_html=True,
    )
    ...
    if st.button("Tailor My Resume", type="primary", key="tailor_btn") and tailor_jd.strip():
        ...
        from claude_ai import decode_jd_claude, explain_skill_gaps_claude, tailor_resume_claude
        from scorer import get_model
    ...
    if st.session_state.get("tailor_results"):
        ...
        vc_color = {"strong": "#059669", "medium": "#d97706", "weak": "#dc2626"}[
            ats_r["verdict"][0]
        ]

        c1, c2, c3 = st.columns(3)
        c1.markdown(
            f"<div style='text-align:center'>{score_badge(ats_r['score'], 56)}<div style='font-size:11px;color:#94a3b8;margin-top:4px'>ATS Score</div></div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            alert(
                ats_r["verdict"][1],
                "green"
                if ats_r["verdict"][0] == "strong"
                else "amber"
                if ats_r["verdict"][0] == "medium"
                else "red",
            ),
            unsafe_allow_html=True,
        )
        ...
        if ats_r.get("found_keywords"):
            st.markdown(
                "<span style='font-size:12px;font-weight:600;color:#059669'>Keywords you already have: </span>"
                + " ".join(chip(k, "green") for k in ats_r["found_keywords"][:10]),
                unsafe_allow_html=True,
            )
        ...
        if missing_hard:
            st.markdown(
                "<span style='font-size:12px;font-weight:600;color:#dc2626'>Hard skills to add: </span>"
                + " ".join(chip(k, "red") for k in missing_hard[:8]),
                unsafe_allow_html=True,
            )
        if missing_soft:
            st.markdown(
                "<span style='font-size:12px;font-weight:600;color:#d97706'>Soft skills to highlight: </span>"
                + " ".join(chip(k, "amber") for k in missing_soft[:6]),
                unsafe_allow_html=True,
            )
            ...
            with st.expander("💡 Why these gaps matter — and how to fix them", expanded=True):
                for g in gap_exps:
                    st.markdown(
                        ...
                        f"<div style='font-weight:700;font-size:13px;color:#fde68a;margin-bottom:4px'>{xe(g.get('skill', ''))}</div>"
                        f"<div style='font-size:12.5px;color:#64748b;margin-bottom:3px'><b style='color:#94a3b8'>Why it matters:</b> {xe(g.get('why', ''))}</div>"
                        f"<div style='font-size:12.5px;color:#6ee7b7'><b style='color:#94a3b8'>How to fix:</b> {xe(g.get('how', ''))}</div>"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
        ...
        if decoded:
            st.markdown("---")
            st.markdown(
                "<div class='section-tag'>JD Decoder — What This Role Really Wants</div>",
                unsafe_allow_html=True,
            )
            ...
            with dc1:
                if decoded.get("what_they_want"):
                    st.markdown(
                        "<div style='font-size:12.5px;font-weight:700;color:#0f172a;margin-bottom:6px'>What they actually need</div>",
                        unsafe_allow_html=True,
                    )
                    for item in decoded["what_they_want"]:
                        st.markdown(
                            f"<div class='card-slate' style='margin-bottom:6px;font-size:13px'>🎯 {xe(item)}</div>",
                            unsafe_allow_html=True,
                        )
                if decoded.get("gaps"):
                    st.markdown(
                        "<div style='font-size:12.5px;font-weight:700;color:#dc2626;margin:12px 0 6px'>Honest gaps — with fixes</div>",
                        unsafe_allow_html=True,
                    )
                    for item in decoded["gaps"]:
                        st.markdown(
                            f"<div class='card-red' style='margin-bottom:6px;font-size:13px'>⚠ {xe(item)}</div>",
                            unsafe_allow_html=True,
                        )
            with dc2:
                if decoded.get("stand_out"):
                    st.markdown(
                        "<div style='font-size:12.5px;font-weight:700;color:#059669;margin-bottom:6px'>How you stand out</div>",
                        unsafe_allow_html=True,
                    )
                    for item in decoded["stand_out"]:
                        st.markdown(
                            f"<div class='card-green' style='margin-bottom:6px;font-size:13px'>✓ {xe(item)}</div>",
                            unsafe_allow_html=True,
                        )
                if decoded.get("questions"):
                    st.markdown(
                        "<div style='font-size:12.5px;font-weight:700;color:#2563eb;margin:12px 0 6px'>Smart questions to ask them</div>",
                        unsafe_allow_html=True,
                    )
                    for item in decoded["questions"]:
                        st.markdown(
                            f"<div class='card-blue' style='margin-bottom:6px;font-size:13px'>💬 {xe(item)}</div>",
                            unsafe_allow_html=True,
                        )
        ...
        if claude_rewrites:
            st.markdown("---")
            st.markdown(
                "<div class='section-tag'>AI-Tailored Resume Rewrites</div>", unsafe_allow_html=True
            )
            st.caption(
                "Your existing bullets rewritten by Claude to match this specific job description. Copy and replace in your resume."
            )
            ...
            st.markdown("---")
            st.markdown(
                "<div class='section-tag'>Before → After: Targeted Rewrites</div>",
                unsafe_allow_html=True,
            )
            st.caption(
                "These are your existing bullets rewritten to include each missing keyword. Copy and replace in your resume."
            )
            for r in results:
                kw_lower = r["kw"].lower()
                orig = r["original"]
            ...
            st.markdown(
                alert(
                    "No weak bullets found that need targeted rewrites — your resume already covers the key terms well.",
                    "green",
                ),
                unsafe_allow_html=True,
            )
...
with tab_examples:
    st.markdown(
        "<div class='page-sub'>Browse real resume examples by role — see what strong summaries and bullets look like, then adapt them for your own experience.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("---")

    role_filter = st.selectbox(
        "Browse examples for:", list(RESUME_EXAMPLES.keys()), key="ex_role_filter"
    )
    ...
    with col_ex:
        ...
        st.markdown(
            "<div class='section-tag'>Strong Experience Bullets</div>", unsafe_allow_html=True
        )
        for b in ex["bullets"]:
            highlighted = re.sub(
                r"(\d+[\w%$KM]*|\$[\d,.]+)", r"<strong style='color:#2563eb'>\1</strong>", b
            )
            st.markdown(
    ...
    with col_tips:
        ...
        st.markdown(
            "<div style='font-size:12.5px;font-weight:700;color:#64748b;margin-bottom:8px;text-transform:uppercase;letter-spacing:.04em'>Summary</div>",
            unsafe_allow_html=True,
        )
        for tip in EXAMPLE_TIPS["summary"]:
            st.markdown(
                f"<div style='font-size:13px;color:#374151;margin-bottom:8px;padding:8px 10px;background:#f0f9ff;border-radius:6px;line-height:1.5'>✓ {tip}</div>",
                unsafe_allow_html=True,
            )
        st.markdown(
            "<div style='font-size:12.5px;font-weight:700;color:#64748b;margin:14px 0 8px;text-transform:uppercase;letter-spacing:.04em'>Bullets</div>",
            unsafe_allow_html=True,
        )
        for tip in EXAMPLE_TIPS["bullets"]:
            st.markdown(
                f"<div style='font-size:13px;color:#374151;margin-bottom:8px;padding:8px 10px;background:#f0fdf4;border-radius:6px;line-height:1.5'>✓ {tip}</div>",
                unsafe_allow_html=True,
            )
        st.markdown(
            "<div style='font-size:12.5px;font-weight:700;color:#64748b;margin:14px 0 8px;text-transform:uppercase;letter-spacing:.04em'>ATS Optimization</div>",
            unsafe_allow_html=True,
        )
        for tip in EXAMPLE_TIPS["ats"]:
            st.markdown(
                f"<div style='font-size:13px;color:#374151;margin-bottom:8px;padding:8px 10px;background:#fff7ed;border-radius:6px;line-height:1.5'>⚡ {tip}</div>",
                unsafe_allow_html=True,
            )
        st.markdown("---")
        st.markdown(
            "<div style='font-size:12px;color:#94a3b8;line-height:1.6'>These examples are based on patterns from strong resumes across industries. Adapt the structure and swap in your real numbers — never copy verbatim.</div>",
            unsafe_allow_html=True,
        )
