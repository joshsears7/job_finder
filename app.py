...
from scorer import get_model, get_skill_gaps, score_job
from utils import (
    _todays_tasks,
    alert,
    chip,
...
""",
    ...
    with demo_col:
            ...
            with st.spinner("Loading demo…"):
                ...
                if res["ok"]:
                    ...
                    try:
                        ...
                        _log.warning("demo resume seed failed: %s", _e)
        ...
        st.markdown(
            "<div style='text-align:center;color:#334155;font-size:11.5px;margin:.5rem 0 1.25rem'>Loads a sample resume &amp; pre-built profile — no sign-up needed</div>",
            unsafe_allow_html=True,
        )
    ...
    with col:
        ...
        with tab_reg:
            ...
            if st.button(
                "Create Account", use_container_width=True, type="primary", key="register_btn"
            ):
...
with st.sidebar:
    ...
    if _sb_profile:
        ...
        _sb_verline = (
            f"<div style='font-size:10px;color:#7c3aed;font-weight:600;margin-top:2px'>v: {xe(_sb_ver)}</div>"
            if _sb_ver
            else ""
        )
        st.markdown(
...
def _try_autoload_resume():
    if st.session_state.get("resume"):
        return
    if st.session_state.get("_skip_autoload"):
        return
    try:
        ...
        _log.warning("autoload resume failed: %s", _e)
    ...
    if all_bullets and has_numbers == 0 and len(all_bullets) >= 4:
        suggestions.append(
            (
                ...
                "None of your bullets currently include numbers. Where it fits naturally, "
...
def infer_job_queries(profile):
    ...
    reasons = []
    if matched_titles:
        reasons.append(f"job title: {matched_titles[0]}")
    if matched_skills:
        ...
        with col_demo:
            ...
            if st.button("Try Demo Resume", use_container_width=True):
                with st.spinner("Analyzing…"):
                    ...
                    st.session_state.pop("_skip_autoload", None)
                st.rerun()
            ...
            try:
                ...
                with st.spinner("Analyzing your resume — loading AI model on first run (30–60 s)…"):
                    ...
                    if not text or len(text.strip()) < 50:
                        st.error(
                            "Could not read text from this file. Try a different PDF or paste your resume."
                        )
                        st.stop()
                    p = parse_resume(text)
                    get_model()
                    ...
                    try:
                        ...
                        _log.warning("profile store save failed: %s", _e)
                    try:
                        ...
                        _log.warning("vector index failed: %s", _e)
                    for k in [
                        "dashboard_jobs",
...
""",
        ...
        for col, icon, title, desc in [
            (
                ...
                "📊",
                "Resume Analyzer",
                "Instant score, section breakdown, bullet coaching, ATS keyword scan, and specific rewrites powered by AI",
            ),
            (
                f2,
                "🔍",
                "Semantic Job Match",
                "AI scores every live job against your resume on meaning — not just keywords. Sorted by real fit.",
            ),
            (
                f3,
                "✍️",
                "23-Tool Writing Suite",
                "Cover letters, cold emails, LinkedIn messages, thank-you notes, essays — all tailored to you and the specific job",
            ),
            (
                f4,
                "📋",
                "Application Tracker",
                "Full pipeline dashboard: saved → applied → interview → offer. Follow-up reminders, contacts, A/B resume testing",
            ),
        ]:
            col.markdown(
        ...
        st.markdown(
            "<div class='section-tag'>What makes CareerIQ different</div>", unsafe_allow_html=True
        )
        st.markdown(
            """
...
""",
        ...
        if hdr_r.button("↺ New Resume", key="dash_new_resume"):
            for k in [
                ...
                st.session_state.pop(k, None)
            st.session_state["_skip_autoload"] = True
            st.rerun()

        dash_tab1, dash_tab2, dash_tab3 = st.tabs(
            ["Resume Health", "Job Matches", "Today's Actions"]
        )
        ...
        with dash_tab1:
            ...
            with col_left:
                st.markdown(
                    "<div class='section-tag' style='margin-top:12px'>Section Breakdown</div>",
                    unsafe_allow_html=True,
                )
                for sec_name, sec_data in analysis["section_analyses"].items():
                    ...
                    issues_html = "".join(
                        f"<div style='font-size:12px;color:#fca5a5;margin-top:2px'>⚠ {i}</div>"
                        for i in sec_data["issues"][:2]
                    )
                    sug_html = "".join(
                        f"<div style='font-size:12px;color:#93c5fd;margin-top:2px'>💡 {sg}</div>"
                        for sg in sec_data["suggestions"][:1]
                    )
                    st.markdown(
                ...
                if analysis["missing_sections"]:
                    st.markdown(
                        "<div class='section-tag' style='margin-top:8px'>Missing Sections</div>",
                        unsafe_allow_html=True,
                    )
                    for sec in analysis["missing_sections"]:
                        tip = analysis["missing_tips"].get(sec, "")
                        st.markdown(
                            alert(f"<b>No {xe(sec)} section</b> — {xe(tip)}", "red"),
                            unsafe_allow_html=True,
                        )
                ...
                if _expl:
                    with st.expander(
                        f"Why {analysis['overall_score']}%? — see what's moving your score",
                        expanded=False,
                    ):
                        st.markdown(
                        ...
                        if _expl.get("next_moves"):
                            st.markdown(
                                "<div style='font-size:11px;font-weight:700;text-transform:uppercase;color:#3b82f6;margin:14px 0 6px;letter-spacing:.06em'>Next moves</div>",
                                unsafe_allow_html=True,
                            )
                            for _mv in _expl["next_moves"]:
                                st.markdown(
                                    f"<div style='font-size:12.5px;color:#93c5fd;margin-bottom:4px'>→ {xe(_mv)}</div>",
                                    unsafe_allow_html=True,
                                )

            with col_right:
                st.markdown(
                    "<div class='section-tag' style='margin-top:12px'>Detected Skills</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    " ".join(chip(s, "blue") for s in profile["skills"]), unsafe_allow_html=True
                )

                _exp_content = analysis["sections"].get("Experience", "")
                if _exp_content:
                    st.markdown(
                        "<div class='section-tag' style='margin-top:16px'>Work Experience</div>",
                        unsafe_allow_html=True,
                    )
                ...
                st.markdown(
                    "<div class='section-tag' style='margin-top:16px'>Top Resume Improvements</div>",
                    unsafe_allow_html=True,
                )
                ...
                if suggestions:
                    ...
                    st.markdown(
                        alert(
                            "Resume looks solid — head to Jobs to find your best matches.", "green"
                        ),
                        unsafe_allow_html=True,
                    )
            ...
            if len(_vault_dash) >= 2 and _vstats_dash:
                st.markdown("---")
                st.markdown(
                    "<div class='section-tag'>Resume A/B Performance</div>", unsafe_allow_html=True
                )
                _best = (
                    max(_vstats_dash, key=lambda v: _vstats_dash[v]["response_rate"])
                    if _vstats_dash
                    else None
                )
                _ab_cols = st.columns(min(len(_vstats_dash), 4))
                for _abi, (_vn, _vs) in enumerate(
                    sorted(_vstats_dash.items(), key=lambda x: -x[1]["response_rate"])
                ):
        ...
        with dash_tab2:
            ...
            if "dashboard_jobs" not in st.session_state:
                with st.spinner("Finding best matches for you…"):
                    ...
                    for j in jobs:
                        j["score"] = score_job(
                            profile["raw_text"], j["description"], j.get("title", "")
                        )
                        j["matched"], j["missing"] = get_skill_gaps(
                            profile["raw_text"], j["description"]
                        )
                ...
                with da:
                    if tracker.is_saved(j["id"]):
                        ...
                        if st.button("Save", key=f"dsv_{j['id']}", use_container_width=True):
                            tracker.save_job(j, s)
                            st.rerun()
                with db:
                    _jurl = str(j.get("url", "")).strip()
                    if _jurl.startswith(("https://", "http://")):
        ...
        with dash_tab3:
            ...
            if _hs["total_applied"] > 0:
                _rr_color = (
                    "#10b981"
                    if _hs["response_rate"] >= 10
                    else "#f59e0b"
                    if _hs["response_rate"] >= 5
                    else "#ef4444"
                )
                _rr_bench = (
                    "above avg ✓"
                    if _hs["response_rate"] >= 5
                    else "below avg — try A/B testing your resume headline"
                )
                _ir_color = (
                    "#10b981"
                    if _hs["interview_rate"] >= 15
                    else "#f59e0b"
                    if _hs["interview_rate"] >= 8
                    else "#ef4444"
                )
                _h1, _h2, _h3, _h4 = st.columns(4)
                _h1.markdown(
                    f"<div class='card' style='text-align:center'><div style='font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase'>Applied</div><div style='font-size:36px;font-weight:900;color:#f1f5f9'>{_hs['total_applied']}</div><div style='font-size:10px;color:#64748b'>{_hs['week_applied']} this week</div></div>",
                    unsafe_allow_html=True,
                )
                _h2.markdown(
                    f"<div class='card' style='text-align:center'><div style='font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase'>Response Rate</div><div style='font-size:36px;font-weight:900;color:{_rr_color}'>{_hs['response_rate']}%</div><div style='font-size:10px;color:#64748b'>{_rr_bench}</div></div>",
                    unsafe_allow_html=True,
                )
                _h3.markdown(
                    f"<div class='card' style='text-align:center'><div style='font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase'>Interviews</div><div style='font-size:36px;font-weight:900;color:{_ir_color}'>{_hs['total_interviews']}</div><div style='font-size:10px;color:#64748b'>{_hs['interview_rate']}% of apps</div></div>",
                    unsafe_allow_html=True,
                )
                _h4.markdown(
                    f"<div class='card' style='text-align:center'><div style='font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase'>Follow-ups Due</div><div style='font-size:36px;font-weight:900;color:{'#ef4444' if _hs['due_followups'] else '#10b981'}'>{_hs['due_followups']}</div><div style='font-size:10px;color:#64748b'>{'action needed' if _hs['due_followups'] else 'all clear'}</div></div>",
                    unsafe_allow_html=True,
                )
                st.markdown("<br>", unsafe_allow_html=True)
            _TASK_STYLE = {
            ...
            _apps_wk = sum(
                ...
                and a["status"] in ("applied", "interview", "offer", "rejected")
            ...
            if _saved_apps and profile:
                st.markdown("---")
                st.markdown(
                    "<div class='section-tag'>Job Search Intelligence</div>", unsafe_allow_html=True
                )
                ...
                _avg_score = int(
                    sum(a.get("score", 0) for a in _saved_apps) / max(len(_saved_apps), 1)
                )
                _best_app = (
                    max(_saved_apps, key=lambda a: a.get("score", 0)) if _saved_apps else None
                )
                ...
                _ic3.markdown(
                    ...
                    f"<div style='font-size:20px;font-weight:800;color:#10b981;margin-top:6px;line-height:1.3'>{xe(_best_app.get('title', '')[:22] + ('…' if len(_best_app.get('title', '')) > 22 else '')) if _best_app else '—'}</div>"
                ...
                _unacted = [
                    a for a in _saved_apps if a["status"] == "saved" and a.get("score", 0) >= 65
                ]
                if _unacted:
