...
import tracker
from utils import alert, chip, inject_css, score_badge, xe

inject_css()

# ── Writing Suite ─────────────────────────────────────────────────
from ai_tools import ats_scan
from writing_suite import PROMPT_CATALOG, generate

st.markdown(
    "<div class='page-title'>Writing Suite</div><div class='page-sub'>AI-assisted writing for every stage of your job search — tailored to your profile.</div>",
...
if not profile:
    st.markdown(
        alert("Load your resume on the Dashboard first to personalize all outputs.", "blue"),
        unsafe_allow_html=True,
    )
    st.stop()
...
with st.expander("⚙️ Job Context (optional — improves every tool)", expanded=False):
    ...
    c1, c2 = st.columns(2)
    manual_company = c1.text_input(
        "Company name",
        value=job_ctx.get("company", "") if job_ctx else "",
        placeholder="e.g. Goldman Sachs",
    )
    manual_role = c2.text_input(
        "Job title",
        value=job_ctx.get("title", "") if job_ctx else "",
    ...
    pasted_jd = st.text_area(
        ...
        height=120,
        placeholder="Paste the full posting for best results…",
    )
...
with col_menu:
    ...
    essay_prompts = {
        k: v for k, v in PROMPT_CATALOG.items() if v["category"] == "Application Essays"
    }
...
with col_output:
    ...
    if not selected:
        ...
        n_offers = st.number_input(
            "How many offers?", min_value=2, max_value=4, value=2, step=1, key="oc_n"
        )
        offers_input = []
        for oi in range(int(n_offers)):
            ...
            bens = st.text_input(
                "Benefits",
                key=f"oc_ben_{oi}",
                placeholder="e.g. 4% 401k match, unlimited PTO, remote",
            )
            notes = st.text_input(
                "Notes", key=f"oc_nt_{oi}", placeholder="e.g. 6-month cliff, on-site 3 days/week"
            )
            offers_input.append(
                {
                    "company": co_nm,
        ...
        if st.session_state.get("ws_output") and st.session_state.ws_output[0] == "_offer_compare":
            ...
            if oc_res and isinstance(oc_res, dict):
                ...
                st.markdown(
                    ...
                    f"<div style='font-size:14px;color:#e2e8f0;line-height:1.6'>{xe(oc_res.get('recommendation', ''))}</div>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
                    ...
                    for bi, bd in enumerate(breakdown):
                        with cols[bi]:
                            ...
                            for p in bd.get("pros", []):
                                st.markdown(
                                    f"<div style='font-size:12px;color:#059669;padding:2px 0'>✅ {xe(p)}</div>",
                                    unsafe_allow_html=True,
                                )
                            for c in bd.get("cons", []):
                                st.markdown(
                                    f"<div style='font-size:12px;color:#dc2626;padding:2px 0'>❌ {xe(c)}</div>",
                                    unsafe_allow_html=True,
                                )
        ...
        st.caption(
            "Claude reviews your resume and application history and gives specific strategic advice — not generic tips."
        )
        ...
        if (
            st.session_state.get("ws_output")
            and st.session_state.ws_output[0] == "_search_strategy"
        ):
            ss_res = st.session_state.ws_output[1]
            if ss_res:
                st.markdown("---")
                st.markdown(
                    "<div class='section-tag'>Your Personalized Strategy</div>",
                    unsafe_allow_html=True,
                )
                for para in ss_res.strip().split("\n\n"):
                    if para.strip():
                        st.markdown(
                            f"<div style='font-size:14px;color:#e2e8f0;line-height:1.7;margin-bottom:16px'>{xe(para.strip())}</div>",
                            unsafe_allow_html=True,
                        )
                st.markdown(
                    alert(
                        "🔁 <b>Tip:</b> Re-run this after every 10 applications — strategy should evolve as you get more data.",
                        "blue",
                    ),
                    unsafe_allow_html=True,
                )
                ...
                ss_pdf = writing_output_pdf(
                    "Job Search Strategy", ss_res, name=profile.get("name", "")
                )
                st.download_button(
                    "⬇ Download Strategy PDF",
                    data=ss_pdf,
                    file_name="CareerIQ_Strategy.pdf",
        ...
        if st.session_state.get("ws_output") and st.session_state.ws_output[0] == "_ats":
            ...
            vc = {"strong": "#059669", "medium": "#d97706", "weak": "#dc2626"}.get(
                r["verdict"][0], "#64748b"
            )
            c1, c2, c3 = st.columns(3)
            c1.markdown(
                f"<div style='text-align:center'>{score_badge(r['score'], 60)}<div style='font-size:11px;color:#64748b;margin-top:4px'>ATS Score</div></div>",
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                alert(
                    r["verdict"][1],
                    "green"
                    if r["verdict"][0] == "strong"
                    else "amber"
                    if r["verdict"][0] == "medium"
                    else "red",
                ),
                unsafe_allow_html=True,
            )
            ...
            with cf:
                ...
                st.markdown(
                    " ".join(chip(s, "green") for s in r["found_keywords"]) or "*None*",
                    unsafe_allow_html=True,
                )
            with cm:
                st.markdown("**🔴 Keywords missing**")
                st.markdown(
                    " ".join(chip(s, "red") for s in r["missing_keywords"]) or "✅ None!",
                    unsafe_allow_html=True,
                )
            if r.get("extra_missing"):
                st.markdown("**🟡 Other JD terms not in resume**")
                st.markdown(
                    " ".join(chip(w, "gray") for w in r["extra_missing"]), unsafe_allow_html=True
                )
            if r["suggested_bullets"]:
                ...
                for item in r["suggested_bullets"]:
                    st.markdown(
                        f"<div class='card-amber'><span style='font-size:11px;font-weight:700;color:#92400e'>ADD: {item['keyword'].upper()}</span><br>• {item['suggested_bullet']}</div>",
                        unsafe_allow_html=True,
                    )
        ...
        if st.session_state.get("ws_output") and st.session_state.ws_output[0] == selected:
            ...
            with tip_col:
                st.markdown(
                    alert(
                        "Select all → Copy → Paste into your application, or download as PDF.",
                        "blue",
                    ),
                    unsafe_allow_html=True,
                )
