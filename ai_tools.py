...
def _top_resume_sentences(resume_text, job_description, top_n=3):
    ...
    sentences = [
        s for s in raw if not _SKIP_PATTERNS.search(s) and not s.isupper() and len(s.split()) >= 6
    ]
    if not sentences:
        return []
...
def generate_cover_letter(profile, job):
    ...
    is_student = any(
        w in raw_lower for w in ["university", "college", "student", "graduating", "gpa"]
    )
    if is_student:
        edu_sentence = (
    ...
    tech_skills = [
        ...
        if s in ("python", "sql", "flask", "pandas", "r programming", "excel", "rest api")
    ]
    ...
    role_value = {
        ...
        role_family,
        f"I am confident my skills in {skills_str} align well with what {company} is looking for.",
    )

    # Assemble paragraphs, skip empty ones
    body_paras = [
        p for p in [edu_sentence, leadership_sentence, project_sentence, role_value] if p.strip()
    ]
    body = "\n\n".join(body_paras)
...
def _build_bullet_suggestion(kw, resume_text):
    ...
    try:
        ...
        sentences = [s for s in sentences if len(s.split()) >= 5 and not _SKIP_PATTERNS.search(s)][
            :60
        ]
        if sentences:
            model = _scorer.get_model()
    ...
    SUGGESTIONS = {
        "python": _ctx(
            "Automated workflow using Python, reducing manual effort and improving consistency"
        ),
        "sql": _ctx(
            "Queried database using SQL to analyze key metrics, surfacing insights that drove a specific decision"
        ),
        "excel": _ctx(
            "Built Excel model tracking KPIs for the team, enabling faster reporting and planning"
        ),
        "tableau": _ctx(
            "Designed Tableau dashboard visualizing performance metrics for stakeholders, enabling data-driven decisions"
        ),
        "data analysis": _ctx(
            "Analyzed dataset to identify trends in a key area, contributing to a measurable business outcome"
        ),
        "project management": _ctx(
            "Managed project from initiation through delivery, coordinating stakeholders and meeting all milestones"
        ),
        "machine learning": _ctx(
            "Applied machine learning model to predict an outcome, improving accuracy over the previous approach"
        ),
        "aws": _ctx(
            "Deployed application on AWS, achieving meaningful improvement in reliability or cost"
        ),
        "marketing": _ctx(
            "Led marketing initiative targeting a specific audience, driving measurable growth in a key metric"
        ),
        "financial modeling": _ctx(
            "Built financial model to forecast revenue or costs, used by leadership to guide a major decision"
        ),
        "communication": _ctx(
            "Presented complex findings to cross-functional stakeholders, aligning teams on a key decision"
        ),
        "leadership": _ctx(
            "Led team through an initiative, setting clear goals and delivering the result on schedule"
        ),
    }

    return SUGGESTIONS.get(
        kw, _ctx(f"Demonstrate {kw}: action verb + specific context + measurable result")
    )
...
def ats_scan(resume_text, job_description):
    ...
    jd_words = set(re.findall(r"\b[a-z][a-z]+\b", jd_lower))
    STOP = {
        "and",
        "the",
        "for",
    ...
    if final_score >= 75:
        verdict = (
            "strong",
            "Strong match. Your resume should pass ATS and reach a human reviewer.",
        )
    elif final_score >= 50:
        ...
        verdict = (
            "weak",
            "Weak match. Consider whether this role fits your current profile, or tailor heavily.",
        )
...
ROLE_QUESTIONS = {
    "analyst": [
        (
            ...
            "Talk about cleaning, exploring distributions, looking for nulls, then forming hypotheses. Mention pandas/SQL/Excel.",
            "Technical",
        ),
        (
            "What metrics would you track to measure the success of [product/campaign]?",
            "Ask clarifying questions first. Then framework: leading vs lagging indicators, business goals.",
            "Technical",
        ),
        (
            "How comfortable are you with SQL? Walk me through a query you've written.",
            "Describe a real query from your actual experience — joins, aggregations, filtering. Be specific about what you queried and what insight you surfaced.",
            "Technical",
        ),
    ],
    "marketing": [
        (
            ...
            "Cover: hypothesis, control/variant, sample size, significance threshold, measurement.",
            "Technical",
        ),
        (
            "What channels would you prioritize for a B2C brand with a limited budget?",
            "SEO, email, and organic social for low-cost. Show you understand CAC and LTV.",
            "Technical",
        ),
        (
            "How do you stay current on marketing trends?",
            "Mention industry newsletters, podcasts, or communities you follow. Name 1-2 specific sources — it signals genuine interest.",
            "Situational",
        ),
    ],
    "finance": [
        (
            ...
            "Project FCFs, terminal value, discount by WACC. Reference any financial modeling you've done — even academic projects count.",
            "Technical",
        ),
        (
            "What's happening in the markets right now that concerns you most?",
            "Pick a real macro theme: interest rates, AI sector valuation, geopolitical risks. Show you follow markets.",
            "Situational",
        ),
        (
            "How would you value a company with negative earnings?",
            "Revenue multiples, EV/EBITDA comps, precedent transactions, option value for early stage.",
            "Technical",
        ),
    ],
    "engineer": [
        (
            ...
            "Cover ingestion, transformation, storage, scheduling, and failure handling. Reference a real system you've built.",
            "Technical",
        ),
        (
            "How do you handle API rate limits in production?",
            "Cover: caching responses, exponential backoff, request queuing, and circuit breakers. Reference a specific API integration you've implemented.",
            "Technical",
        ),
        (
            "What's your approach to debugging a system that's silently failing?",
            "Cover: structured logging, monitoring/alerting, unit tests for edge cases, and reproducing the failure locally.",
            "Technical",
        ),
    ],
    "operations": [
        (
            ...
            "Eisenhower matrix or impact/effort framework. Show structured thinking.",
            "Situational",
        ),
        (
            "Describe a process you identified as inefficient and how you improved it.",
            "Use STAR format. Think about: a reporting process you automated, a workflow you streamlined, or a recurring task you improved. Quantify time saved if possible.",
            "Behavioral",
        ),
    ],
    "general": [
        (
            ...
            "Research the company before. Mention something specific — a product, mission, recent news.",
            "Situational",
        ),
        (
            "Where do you see yourself in 5 years?",
            "Show ambition + relevance to the role. Don't be too specific — growth within the field.",
            "Situational",
        ),
    ],
}
...
CLOSING_QUESTIONS = [
    (
        ...
        "Lead with your top 2-3 differentiators: a specific technical skill, a demonstrated result, and something about your approach that sets you apart. Be direct and confident — this is not the time to hedge.",
        "Closing",
    ),
    (
        "Do you have any questions for us?",
        "Always ask at least 2. Good ones: 'What does success look like in the first 90 days?' and 'What's the team's biggest challenge right now?'",
        "Closing",
    ),
]
