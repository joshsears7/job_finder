...
def get_macro_snapshot():
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        futures = {pool.submit(_fetch_fred, sid, 3): name for name, sid in MACRO_SERIES.items()}
...
def get_sector_changes():
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        futures = {pool.submit(_fetch_fred, sid, 3): name for name, sid in SECTOR_SERIES.items()}
        for fut in as_completed(futures):
            name = futures[fut]
            data = fut.result()
            if len(data) < 2:
...
def get_job_news():
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        futures = {pool.submit(_fetch_rss, url, 6): name for name, url in NEWS_FEEDS}
        for fut in as_completed(futures):
            source = futures[fut]
