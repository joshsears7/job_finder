...
st.markdown(
    "<div class='page-title'>My Profile</div><div class='page-sub'>Your personal info, job search preferences, and background scanner settings.</div>",
...
tab_info, tab_search, tab_scanner, tab_alerts = st.tabs(
    ["Personal Info", "Job Search", "Scanner Config", "Job Alerts"]
)
...
with tab_info:
    ...
    location = c2.text_input(
        "Current location", value=profile.get("current_location", ""), key="prof_location"
    )

    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Online Presence</div>",
        unsafe_allow_html=True,
    )
    c3, c4 = st.columns(2)
    linkedin = c3.text_input(
        "LinkedIn URL", value=profile.get("linkedin_url", ""), key="prof_linkedin"
    )
    github = c4.text_input("GitHub URL", value=profile.get("github_url", ""), key="prof_github")
    portfolio = c3.text_input(
        "Portfolio URL", value=profile.get("portfolio_url", ""), key="prof_portfolio"
    )

    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Education & Background</div>",
        unsafe_allow_html=True,
    )
    c5, c6 = st.columns(2)
    ...
    grad = c6.text_input(
        "Graduation date", value=profile.get("graduation_date", ""), key="prof_grad"
    )
    work_auth = c5.text_input(
        "Work authorization", value=profile.get("work_auth", ""), key="prof_auth"
    )
    yrs_exp = c6.number_input(
        "Years of experience",
        min_value=0,
        max_value=50,
        value=int(profile.get("years_experience", 0) or 0),
        key="prof_yrs",
    )
...
with tab_search:
    ...
    job_type = c1.selectbox(
        "Job type",
        ["internship", "full-time", "both"],
        index=["internship", "full-time", "both"].index(profile.get("job_type", "internship"))
        if profile.get("job_type") in ["internship", "full-time", "both"]
        else 0,
        key="prof_job_type",
    )
    salary_type = c2.selectbox(
        ...
        if profile.get("salary_type") in ["hourly", "annual"]
        else 0,
        key="prof_sal_type",
    )
    open_remote = c3.checkbox(
        "Open to remote", value=bool(profile.get("open_to_remote", True)), key="prof_remote"
    )
    c4, c5, c6 = st.columns(3)
    min_sal = c4.number_input(
        "Min salary",
        min_value=0,
        max_value=500000,
    ...
    open_relocate = c6.checkbox(
        "Open to relocate", value=bool(profile.get("open_to_relocate", True)), key="prof_relocate"
    )

    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Target Roles</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Target Cities</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Target Companies</div>",
        unsafe_allow_html=True,
    )
    c7, c8 = st.columns(2)
    targets_text = c7.text_area(
        "Companies I want to work at (one per line)",
        value="\n".join(profile.get("target_companies", [])),
        height=120,
        key="prof_targets",
    )
    blacklist_text = c8.text_area(
        "Companies to skip (one per line)",
        ...
        profile.update(
            {
                ...
                "blacklist_companies": [
                    c.strip() for c in blacklist_text.splitlines() if c.strip()
                ],
            }
        )
        ps.save_profile(profile, user_id)
...
with tab_scanner:
    st.markdown(
        "<div class='section-tag'>Background Scanner Settings</div>", unsafe_allow_html=True
    )
    st.caption(
        "The background scanner runs on a schedule to find new high-fit jobs and notify you. Configure thresholds here."
    )
    ...
    scan_interval = c1.selectbox(
        ...
        if int(profile.get("scan_interval_hours", 4) or 4) in [1, 2, 4, 6, 12, 24]
        else 2,
...
with tab_alerts:
    ...
    from job_alerts import load_config as _al_load
    from job_fetcher import CITY_PRESETS

    st.markdown("<div class='section-tag'>Push Notifications (ntfy)</div>", unsafe_allow_html=True)
    st.caption(
        "ntfy is a free, open-source push notification service. No account needed — just pick a topic name."
    )
    ...
    if c_ntfy2.button("Test Notification", key="al_ntfy_test", use_container_width=True):
        if ntfy_topic.strip():
            ...
            if ok:
                st.success(
                    f"Test sent! Open ntfy.sh/{ntfy_topic.strip()} or the ntfy app to confirm."
                )
            else:
    ...
    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Email Alerts</div>",
        unsafe_allow_html=True,
    )
    st.caption(
        "Get an email when a high-match job is found. Requires a free SendGrid API key (sendgrid.com) set as SENDGRID_API_KEY in your environment."
    )
    ...
    if _has_sg and c_email2.button("Test Email", key="al_email_test", use_container_width=True):
        if _alert_email.strip():
            ...
            st.success("Test email sent — check your inbox.") if ok else st.error(
                "Send failed — verify your SENDGRID_API_KEY and email address."
            )
        else:
    ...
    st.caption(
        "Each alert defines a role + cities to watch. You'll be notified when new matching jobs appear above your fit threshold."
    )

    alerts = al_cfg.get("alerts", [])

    with st.expander("➕ Add a new alert", expanded=not bool(alerts)):
        al_role = st.text_input(
            "Role to watch",
            placeholder="e.g. Business Analyst, Data Analyst Intern",
            key="al_new_role",
        )
        city_keys = list(CITY_PRESETS)
        ...
        if st.button("Add Alert", type="primary", key="al_add_btn"):
            if al_role.strip() and al_cities:
                ...
                st.success(
                    f"Alert added: '{al_role.strip()}' in {len(al_cities)} cities (score ≥ {al_min}%)"
                )
                st.rerun()
            else:
    ...
    if not alerts:
        ...
        for i, a in enumerate(alerts):
            ...
            with al2:
                if st.button(
                    "🗑 Remove", key=f"al_del_{i}_{role_str[:15]}", use_container_width=True
                ):
