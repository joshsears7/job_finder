...
def _looks_like_section_header(stripped: str) -> bool:
    ...
    return not sentence_word_hits > 1
...
# Matches concrete quantified results (numbers, percentages, dollars, word-form quantities)
_YEAR_RE = re.compile(r"\b(19|20)\d{2}\b")  # years like 2023 are not metrics
_METRIC_RE = re.compile(
    r"\d+\s*%"  # percentages: 18%, 3.2%
    r"|\$\s*[\d,]+"  # dollar amounts: $1.2M, $500
    r"|\b\d+[km]\b"  # 50k, 3m (must have k/m suffix)
    r"|\b[1-9]\d{0,2}\b(?!\s*(?:19|20)\d{2})"  # 2-3 digit counts (not years)
    r"|\b(millions?|billions?|thousands?|hundreds?|dozens?)\b"  # word-form large quantities
    r"|\btop\s+\d+\b"  # "top 5", "top 10"
    r"|\b\d+x\b"  # multipliers: 3x, 10x
    r"|\b[2-9]x\s+(?:faster|larger|more|better|growth)\b",  # "5x faster"
    re.IGNORECASE,
)


def _has_metric(text: str) -> bool:
    """True if text contains a real quantified result (not just a year)."""
...
def _score_bullet(text: str) -> dict:
    ...
    if _ACTION_VERB_RE.match(cleaned):
        ...
        if any(lower_start.startswith(w) for w in _WEAK_VERB_STARTS):
            score -= 15
            issues.append(
                "Starts with a weak verb — replace with an action verb (Drove, Built, Reduced…)"
            )
        else:
            score -= 5
            issues.append("Lead with a strong action verb (Delivered, Analyzed, Managed…)")

    # +20 for a real quantified metric; +8 for directional verb only (improvement implied but unquantified)
    if _has_metric(cleaned):
        score += 20
    elif _DIRECTIONAL_RE.search(cleaned):
        score += 8
        issues.append(
            "Good direction — adding a number would make this even stronger (e.g. 'by 30%' or '2x')"
        )
    else:
        score -= 8
        issues.append(
            "Add a quantified result — numbers make bullets 40% more memorable to recruiters"
        )
    ...
    if re.search(r"\b(was|were|been)\s+\w+ed\b", cleaned, re.IGNORECASE):
        score -= 5
        issues.append(
            "Passive voice detected — rewrite in active voice ('Managed' not 'Was responsible for managing')"
        )

    # +10 for scope/context words (expanded to include more qualitative excellence signals)
    if re.search(
        r"\b(team|cross-functional|stakeholder|company-wide|organization|department|"
        r"million|thousand|executive|leadership|strategy|initiative|program|portfolio|"
        r"enterprise|global|nationwide|client|customer|revenue|budget|deadline)\b",
        cleaned,
        re.IGNORECASE,
    ):
        score += 10
    ...
    if issues:
        ...
        suggestion = (
            "Strong bullet — consider adding scope (team size, budget, or timeline) if missing."
        )
...
def _score_section(name: str, content: str, all_sections: dict) -> dict:
    ...
    if name == "Summary":
        ...
        if word_count < 8:
            score -= 20
            issues.append(
                "Summary is too short — write 1–2 sentences that position you specifically"
            )
        elif word_count <= 55:
            ...
            score -= 15
            issues.append(
                "Summary is too long — trim to 1–2 sentences; paragraph summaries get skipped"
            )
        ...
        if re.search(
            ...
            score -= 15
            issues.append(
                "Remove overused buzzwords — 'results-driven', 'passionate', etc. say nothing specific"
            )
        else:
            score += 10
        ...
        has_specific = bool(
            re.search(
                r"\b(python|fastapi|llm|rag|api|sql|aws|react|docker|gpt|claude|"
        ...
        if len(bullet_lines) < 3:
            score -= 20
            issues.append(
                f"Only {len(bullet_lines)} bullet point(s) detected — aim for 3–6 per role"
            )
        else:
            score += 20
        has_metrics = any(_has_metric(b) for b in bullet_lines)
        if not has_metrics:
            score -= 15
            issues.append(
                "No quantified results found — add percentages, dollar figures, or counts"
            )
        else:
            score += 15
        ...
        skill_items = [
            s.strip() for s in re.split(r"[,|]", skill_text) if s.strip() and len(s.strip()) > 1
        ]
        ...
        if skill_count < 6:
            ...
            score -= 10
            suggestions.append(
                "Trim to 20–25 skills — prioritize the most in-demand for your target roles"
            )
        else:
            score += 20
        ...
        if re.search(
            ...
            suggestions.append(
                "Organize skills into labeled categories (e.g. Technical, Business, Certifications)"
            )
        ...
        if re.search(r"(github|gitlab|huggingface|hf\.co|demo|live|http)", content, re.IGNORECASE):
            ...
            if re.search(r"huggingface\.co/spaces|hf\.co/spaces", content, re.IGNORECASE):
                suggestions.append(
                    "Live demo on HuggingFace Spaces — strong signal for AI/ML roles"
                )
        else:
            suggestions.append(
                "Add a live demo link or GitHub repo — inline in the bullet, not a separate section"
            )
...
def _calc_dimensions(sections: dict, bullets: list, missing: list) -> dict:
    ...
    if real_bullets:
        metriced = sum(1 for b in real_bullets if _has_metric(b["text"]))
...
def _build_action_plan(section_analyses: dict, bullets: list, dims: dict, missing: list) -> dict:
    quick_wins = []
    high_impact = []
...
def _build_score_explanation(
    section_analyses: dict, missing: list, bullet_analyses: list, overall: int
) -> dict:
    """
            ...
            drivers.append(
                {
                    ...
                    else "Well-structured and complete.",
                }
            )
    ...
    q_bullets = [b for b in real_bullets if _has_metric(b["text"])]
    ...
    if placeholder_bullets:
        ...
        next_moves.append(
            f"Fill in {len(placeholder_bullets)} template placeholder bullet{'s' if len(placeholder_bullets) > 1 else ''} with real achievements."
        )
    ...
    if weak_bullets:
        ...
        next_moves.append(
            f"Rewrite {len(weak_bullets)} weak bullet{'s' if len(weak_bullets) > 1 else ''} — use the AI Rewrite button in Bullet Coach."
        )
        ...
        drivers.append(
            {
                ...
                "detail": SECTION_COACH.get(sec, "").split(".")[0] + "."
                if SECTION_COACH.get(sec)
                else "",
            }
        )
...
def full_analysis(text: str) -> dict:
    ...
    score_explanation = _build_score_explanation(
        section_analyses, missing, bullet_analyses, overall
    )
