...
import tracker
from utils import STATUS_COLOR, STATUS_EMOJI, alert, inject_css, score_color, xe

inject_css()

st.markdown(
    "<div class='page-title'>Track Applications</div><div class='page-sub'>Pipeline Kanban, follow-up schedule, and your network CRM — all in one place.</div>",
...
with tab_pipeline:
    ...
    if not apps:
        ...
        search_query = fc2.text_input(
            "Search by company or title", placeholder="Goldman, analyst…", key="pipeline_search"
        )
        ...
        st.markdown(
            f"<div style='font-size:12px;color:#64748b;margin-bottom:12px'>{len(filtered)} application{'s' if len(filtered) != 1 else ''}</div>",
            unsafe_allow_html=True,
        )
...
with tab_followup:
    ...
    if not due:
        st.markdown(
            alert(
                "No follow-ups due — you're on top of it. Follow-ups auto-schedule 7 days after you mark a job 'Applied'.",
                "green",
            ),
            unsafe_allow_html=True,
        )
        ...
        for fu in due:
            ...
            _ai_draft_key = f"fu_ai_{fu['followup_id']}"
            default_draft = (
                st.session_state.get(_ai_draft_key)
                or fu.get("draft_text", "")
                or (
            ...
            draft = st.text_area(
                ...
                value=default_draft,
                height=160,
                key=f"fu_draft_{fu['followup_id']}",
            )

            fu_c1, fu_c2, fu_c3 = st.columns([1, 1, 1])
            with fu_c1:
                if st.button(
                    "✅ Mark Sent",
                    key=f"fu_done_{fu['followup_id']}",
            ...
            with fu_c2:
                if st.button(
                    "⏭ Skip", key=f"fu_skip_{fu['followup_id']}", use_container_width=True
                ):
                    tracker.skip_followup(fu["followup_id"])
                    st.rerun()
            with fu_c3:
                if st.button(
                    "AI Draft", key=f"fu_ai_btn_{fu['followup_id']}", use_container_width=True
                ):
                    with st.spinner("Writing follow-up…"):
                        from claude_ai import generate_followup_claude
                        _profile = st.session_state.get("resume", {})
                        _name = _profile.get("name", "") if _profile else ""
                        _resume_text = _profile.get("raw_text", "") if _profile else ""
                        _generated = generate_followup_claude(
                            title=fu["title"],
                            company=fu["company"],
                            date_applied=fu.get("date_applied", "")[:10] if fu.get("date_applied") else "",
                            candidate_name=_name,
                            resume_text=_resume_text,
                        )
                        if _generated:
                            st.session_state[_ai_draft_key] = _generated
                            st.rerun()
    ...
    with st.expander("View full follow-up schedule"):
        ...
        if all_fu:
            for fu in all_fu:
                st_icon = (
                    "✅" if fu["status"] == "sent" else "⏭" if fu["status"] == "skipped" else "🕐"
                )
                color = (
                    "#10b981"
                    if fu["status"] == "sent"
...
with tab_crm:
    ...
    st.caption(
        "Track contacts, referrals, and outreach. 70% of jobs are filled through network — most people track none of it."
    )
    ...
    with st.expander("➕ Add a contact", expanded=False):
        ...
        crm_how_met = c4.text_input(
            "How you met", placeholder="LinkedIn, career fair, class…", key="crm_how_met"
        )
        crm_email = c5.text_input("Email", placeholder="sarah@gs.com", key="crm_email")
        crm_linkedin = c6.text_input(
            "LinkedIn URL", placeholder="linkedin.com/in/…", key="crm_linkedin"
        )
        c7, c8 = st.columns(2)
        crm_status = c7.selectbox("Status", CONTACT_STATUSES, key="crm_status")
        crm_next = c8.text_input(
            "Next action",
            placeholder="Send follow-up, ask for intro to hiring team…",
            key="crm_next",
        )
        ...
        if st.button("Save Contact", type="primary", key="crm_save"):
            if crm_name.strip():
                tracker.save_contact(
                    ...
                    how_met=crm_how_met,
                    email=crm_email,
                    linkedin=crm_linkedin,
                    status=crm_status,
                    next_action=crm_next,
                    notes=crm_notes,
                )
    ...
    if not contacts:
        st.markdown(
            alert(
                "No contacts yet. Add people from LinkedIn, career fairs, alumni events, and informational interviews.",
                "blue",
            ),
            unsafe_allow_html=True,
        )
        ...
        shown = (
            contacts if crm_filter == "All" else [c for c in contacts if c["status"] == crm_filter]
        )

        st.markdown(
            f"<div style='font-size:12px;color:#64748b;margin-bottom:10px'>{len(shown)} contact{'s' if len(shown) != 1 else ''}</div>",
            unsafe_allow_html=True,
        )
        ...
        for c in shown:
            ...
            with st.expander(f"{xe(c['name'])} — {xe(c.get('company', ''))} · {xe(c['status'])}"):
                ...
                new_status = ec1.selectbox(
                    "Status",
                    CONTACT_STATUSES,
                    index=CONTACT_STATUSES.index(c["status"])
                    if c["status"] in CONTACT_STATUSES
                    else 0,
                    key=f"crm_st_{c['id']}",
                )
                new_next = ec2.text_input(
                    "Next action", value=c.get("next_action", "") or "", key=f"crm_next_{c['id']}"
                )
                new_notes = st.text_area(
                    "Notes", value=c.get("notes", "") or "", height=80, key=f"crm_notes_{c['id']}"
                )
                ...
                st.markdown(
                    ...
                    + (
                        f" · <a href='{xe(c['linkedin'])}' target='_blank' style='color:#3b82f6'>LinkedIn</a>"
                        if c.get("linkedin")
                        and str(c["linkedin"]).startswith(("https://", "http://"))
                        else ""
                    )
                    + (
                        f" · <a href='mailto:{xe(c['email'])}' style='color:#3b82f6'>{xe(c['email'])}</a>"
                        if c.get("email")
                        else ""
                    )
                    + "</div>",
                ...
                with btn1:
                    if st.button(
                        ...
                        tracker.update_contact(
                            c["id"], status=new_status, next_action=new_next, notes=new_notes
                        )
                        st.success("Updated.")
                        st.rerun()
                with btn2:
