...
from utils import alert, chip, inject_css, xe
...
company_name = c1.text_input(
    "Company name",
    value=prefill.get("company", "") if prefill else "",
    placeholder="e.g. Stripe",
    key="cr_company",
)
role_name = c2.text_input(
    "Role you're applying for",
    value=prefill.get("role", "") if prefill else "",
...
if not dossier:
    st.markdown(
        alert(
            ...
            "then Claude Sonnet synthesizes everything into a tailored brief.",
            "blue",
        ),
        unsafe_allow_html=True,
    )
...
m3.markdown(
    f"<div style='{_metric_style}'>"
    f"<div style='font-size:20px;font-weight:900;color:#10b981'>{xe(str(cb.get('funding', '—')))}</div>"
...
m4.markdown(
    f"<div style='{_metric_style}'>"
    f"<div style='font-size:22px;font-weight:900;color:#7c3aed'>{xe(str(hiring.get('open_roles', '—')))}</div>"
...
with tab_brief:
    ...
    if talking:
        st.markdown(
            "<div class='section-tag'>Talking Points to Reference</div>", unsafe_allow_html=True
        )
        st.caption(
            "Mention these in your cover letter and interviews to show you've done your homework."
        )
        for pt in talking:
            st.markdown(
    ...
    if qs:
        st.markdown(
            "<div class='section-tag' style='margin-top:20px'>Likely Interview Questions</div>",
            unsafe_allow_html=True,
        )
    ...
    if rec_angle:
        st.markdown(
            "<div class='section-tag' style='margin-top:20px'>Recruiter Outreach Angle</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
...
with tab_intel:
    ...
    if why:
        st.markdown(
            "<div class='section-tag' style='margin-top:18px'>Why You Fit Here</div>",
            unsafe_allow_html=True,
        )
        for item in why:
            st.markdown(
    ...
    if red_flags:
        st.markdown(
            "<div class='section-tag' style='margin-top:18px'>Red Flags to Consider</div>",
            unsafe_allow_html=True,
        )
...
with tab_news:
    ...
    if news:
        ...
        for a in news:
            ...
            link_html = (
                f"<a href='{xe(url)}' target='_blank' style='color:#93c5fd;font-weight:600;"
                f"font-size:13.5px;text-decoration:none'>{title}</a>"
                if url.startswith("http")
                else f"<span style='color:#e2e8f0;font-weight:600;font-size:13.5px'>{title}</span>"
            )
            st.markdown(
    ...
    if hn:
        st.markdown(
            "<div class='section-tag' style='margin-top:18px'>HackerNews Mentions</div>",
            unsafe_allow_html=True,
        )
        for item in hn:
            ...
            link_html = (
                ...
                if url.startswith("http")
                else f"<span style='color:#e2e8f0;font-size:13px'>{title}</span>"
            )
            st.markdown(
...
with tab_tech:
    ...
    if tech:
        ...
        st.caption(
            "No tech signals detected — provide the company website URL for tech stack analysis."
        )

    if hiring_roles:
        st.markdown(
            "<div class='section-tag' style='margin-top:18px'>Current Open Roles</div>",
            unsafe_allow_html=True,
        )
