...
st.markdown(
    "<div class='page-title'>Interview Prep</div><div class='page-sub'>Tailored question banks, STAR story builder, salary coaching, and smart questions to ask your interviewer.</div>",
...
if not profile:
    st.markdown(
        alert(
            "Upload your resume on the Dashboard first — questions are tailored to your background.",
            "blue",
        ),
        unsafe_allow_html=True,
    )
...
with tab_qs:
    ...
    job_list = [
        (a["title"], a["company"], a.get("url", ""), "")
        for a in all_saved
        if a["status"] in ("applied", "interview", "saved")
    ]
    ...
    use_saved = st.checkbox(
        "Use a saved/applied job for context", value=bool(job_list), key="iq_use_saved"
    )
    job_ctx = None
    ...
    c1, c2 = st.columns(2)
    manual_title = c1.text_input(
        "Job title",
        value=job_ctx["title"] if job_ctx else "",
        placeholder="e.g. Marketing Analyst Intern",
        key="iq_title",
    )
    manual_company = c2.text_input(
        "Company",
        value=job_ctx["company"] if job_ctx else "",
        placeholder="e.g. Deloitte",
        key="iq_company",
    )
    jd_paste = st.text_area(
        "Paste job description (optional — improves question relevance)",
        value=job_ctx.get("description", "") if job_ctx else "",
        height=100,
        key="iq_jd",
    )
    ...
    if st.button("Generate Question Bank", type="primary", key="gen_qs"):
        with st.spinner(
            "Building tailored question bank — loading AI model on first run (30–60 s)…"
        ):
    ...
    if qs:
        try:
            ...
            safe_co = (manual_company or (job_ctx["company"] if job_ctx else "questions")).replace(
                " ", "_"
            )[:20]
            st.download_button(
        ...
        for qtype, items in grouped.items():
            if qtype in type_order:
                continue
            st.markdown(
                f"<div style='font-size:11px;font-weight:800;text-transform:uppercase;color:#64748b;margin:16px 0 8px'>── {xe(qtype)} ──</div>",
                unsafe_allow_html=True,
            )
            for item in items:
                with st.expander(item["question"]):
                    st.markdown(
                        f"<div style='font-size:13px;color:#93c5fd;line-height:1.65'>💡 {xe(item['hint'])}</div>",
                        unsafe_allow_html=True,
                    )
...
with tab_star:
    ...
    st.caption(
        "STAR = Situation · Task · Action · Result. Prepare 5–7 of these before any interview. They cover 80% of behavioral questions."
    )
    ...
    with st.expander(
        "➕ Add a new STAR story", expanded=not bool(st.session_state["star_stories"])
    ):
        story_title = st.text_input(
            "Story title / theme",
            placeholder="e.g. Led a team project under deadline pressure",
            key="star_title",
        )
        col_s, col_t = st.columns(2)
        situation = col_s.text_area(
            "Situation",
            height=120,
            placeholder="Set the scene. What was the context? What was at stake?",
            key="star_situation",
        )
        task = col_t.text_area(
            "Task",
            height=120,
            placeholder="What was your specific responsibility? What needed to happen?",
            key="star_task",
        )
        col_a, col_r = st.columns(2)
        action = col_a.text_area(
            "Action",
            height=120,
            placeholder="What did YOU specifically do? Use 'I', not 'we'. Be specific.",
            key="star_action",
        )
        result = col_r.text_area(
            "Result",
            height=120,
            placeholder="What was the outcome? Quantify if possible. What did you learn?",
            key="star_result",
        )
    ...
    if stories:
        try:
            ...
            pass
        st.markdown(
            f"<div class='section-tag' style='margin-top:16px'>Your Stories ({len(stories)})</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            alert("No stories yet. Add your first STAR story above.", "blue"),
            unsafe_allow_html=True,
        )
...
with tab_salary:
    st.markdown(
        "<div class='section-tag'>How to Handle 'What Are Your Salary Expectations?'</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown("---")
    st.markdown(
        "<div class='section-tag'>Answering 'What's Your Weakness?'</div>", unsafe_allow_html=True
    )
    st.markdown(
...
with tab_ask:
    st.markdown(
        "<div class='section-tag'>Smart Questions to Ask Your Interviewer</div>",
        unsafe_allow_html=True,
    )
    st.caption(
        "Always ask 2–3 questions. It signals engagement, helps you evaluate the role, and keeps you memorable."
    )
...
with tab_debrief:
    st.markdown(
        "<div class='page-sub'>Turn your interview notes into a specific, ready-to-send thank-you note — not a generic template.</div>",
        unsafe_allow_html=True,
    )

    _db_c1, _db_c2 = st.columns(2)
    _db_interviewer = _db_c1.text_input(
        "Interviewer name", placeholder="Sarah Johnson", key="db_interviewer"
    )
    _db_role = _db_c2.text_input(
        "Role you interviewed for", placeholder="Data Analyst", key="db_role"
    )
    ...
    if st.button(
        "Generate Thank-You Note", type="primary", key="db_gen_btn", use_container_width=True
    ):
        if not _db_topics.strip():
            st.error(
                "Add at least a few notes about what you discussed — that's what makes the note specific."
            )
        else:
            from claude_ai import stream_thankyou_claude

            st.session_state.pop("db_ty_result", None)
            _role_ctx = _db_role or job.get("title", "the role")
            _co_ctx = _db_company or job.get("company", "the company")
            streamed_ty = st.write_stream(
                stream_thankyou_claude(
    ...
    if _ty_out:
        ...
        st.markdown("---")
        st.markdown(
            "<div class='section-tag'>Post-Interview Debrief Checklist</div>",
            unsafe_allow_html=True,
        )
        _checklist = [
            ("Send thank-you note within 24 hours", bool(_ty_out)),
        ...
        st.markdown(
            alert(
                "Fill in the fields above and click Generate — the more specific your notes, the better the thank-you note.",
                "blue",
            ),
            unsafe_allow_html=True,
        )
