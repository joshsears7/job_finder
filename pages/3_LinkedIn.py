...
from linkedin_editor import (
    ...
    generate_headlines,
    generate_salary_negotiation,
    rewrite_bullets_for_linkedin,
    skills_to_add,
)
from utils import alert, chip, inject_css, progress_bar, score_badge, score_color, xe

inject_css()

st.markdown(
    "<div class='page-title'>LinkedIn Optimizer</div><div class='page-sub'>Generate headlines, About sections, cold DMs, and salary scripts — all tailored to your resume. Profile Score is optional.</div>",
...
if not profile:
    st.markdown(
        alert(
            "Upload your resume on the Dashboard first — all outputs are tailored to your background.",
            "blue",
        ),
        unsafe_allow_html=True,
    )
...
with tab_score:
    st.markdown(
        "<div class='section-tag'>Paste your LinkedIn profile text</div>", unsafe_allow_html=True
    )
    st.caption(
        "On LinkedIn → go to your profile → select all text (Cmd+A) → copy (Cmd+C) → paste below. The other tabs don't require this."
    )
    ...
    target_role_score = c1.text_input(
        "Target role (optional)", placeholder="e.g. Business Analyst", key="li_score_role"
    )
    has_photo = c2.checkbox("I have a profile photo", value=True)
    conn_count = c3.number_input(
        "Connections", min_value=0, max_value=30000, value=0, step=50, key="li_conn"
    )
    ...
    if result:
        ...
        with col_l:
            st.markdown(
                "<div class='section-tag' style='margin-top:14px'>Section Scores</div>",
                unsafe_allow_html=True,
            )
            for sec_name, sec_data in result["sections"].items():
                ...
                issues_html = "".join(
                    f"<div style='font-size:12px;color:#fca5a5;margin-top:2px'>⚠ {xe(i)}</div>"
                    for i in sec_data["issues"][:1]
                )
                tip_html = "".join(
                    f"<div style='font-size:12px;color:#93c5fd;margin-top:2px'>💡 {xe(t)}</div>"
                    for t in sec_data["tips"][:1]
                )
                st.markdown(
        ...
        with col_r:
            st.markdown(
                "<div class='section-tag' style='margin-top:14px'>All-Star Checklist</div>",
                unsafe_allow_html=True,
            )
            for item in result["all_star_checklist"]:
                ...
                st.markdown(
                    f"<div style='font-size:13px;color:{color};margin-bottom:5px'>{icon} {xe(item['item'])}</div>",
                    unsafe_allow_html=True,
                )

            if result["top_recommendations"]:
                st.markdown(
                    "<div class='section-tag' style='margin-top:14px'>Top Recommendations</div>",
                    unsafe_allow_html=True,
                )
...
with tab_headlines:
    st.markdown(
        "<div class='section-tag'>Generate LinkedIn Headlines</div>", unsafe_allow_html=True
    )
    st.caption(
        "LinkedIn shows your headline in recruiter search results. It's your #1 searchability lever."
    )
    ...
    for i, h in enumerate(headlines):
        _hl_over = len(h) > 220
        _hl_color = "#ef4444" if _hl_over else "#475569"
        _hl_warn = " ⚠ Over LinkedIn's 220-char limit" if _hl_over else ""
        st.markdown(
            ...
            f"<div style='font-size:11px;color:{_hl_color};margin-top:6px'>{len(h)}/220 characters{_hl_warn}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
with tab_about:
    st.markdown(
        "<div class='section-tag'>Generate LinkedIn About Section</div>", unsafe_allow_html=True
    )
    st.caption(
    ...
    if not profile:
        ...
        with col_bullets:
            st.caption(
                "LinkedIn bullets should be shorter and more achievement-focused than resume bullets."
            )
            ...
            bullets = [
                ...
                if len(b.strip()) > 30
                and b.strip()[0] in "•·-*ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
            ][:12]
...
with tab_skills:
    st.markdown(
        "<div class='section-tag'>Skills to Add to Your LinkedIn</div>", unsafe_allow_html=True
    )
    st.caption(
...
with tab_msgs:
    ...
    with msg_tab1:
        st.markdown(
            "<div class='section-tag'>Connection Request (< 300 characters)</div>",
            unsafe_allow_html=True,
        )
        ...
        cr_company = c_cr2.text_input(
            "Their company", placeholder="Goldman Sachs", key="cr_company"
        )
        cr_context = st.text_input(
            "Context (optional)",
            placeholder="Found you through X posting, alumni network, etc.",
            key="cr_context",
        )
        cr_my_role = st.text_input(
            "Your role/goal", placeholder="e.g. aspiring data analyst", key="cr_my_role"
        )
...
with tab_negotiate:
    ...
    offer_amt = c_n1.number_input(
        "Current offer ($)", min_value=0, max_value=500000, value=65000, step=1000, key="neg_offer"
    )
    target_amt = c_n2.number_input(
        "Your target ($)", min_value=0, max_value=500000, value=75000, step=1000, key="neg_target"
    )
    ...
    if st.button("Generate Negotiation Scripts", type="primary", key="gen_neg"):
        if offer_amt == 0:
            st.warning("Enter the offer amount to get negotiation scripts.")
        elif target_amt <= offer_amt:
            st.error("Your target must be higher than the current offer.")
        else:
            with st.spinner("Building your negotiation playbook…"):
                scripts = generate_salary_negotiation(
                    offer_amt, target_amt, neg_co, neg_role, name_neg
                )
    ...
    if scripts:
        ...
        with st.expander("✉️ Counter-Offer Email"):
            st.text_area(
                "Counter email", value=scripts["counter_email"], height=260, key="neg_email_display"
            )

        with st.expander("🎯 Verbal Script (exact words to say)"):
            st.text_area(
                "Verbal script",
                value=scripts["verbal_script"],
                height=220,
                key="neg_verbal_display",
            )

        with st.expander("💡 Negotiation Tips"):
            for tip in scripts["notes"]:
                st.markdown(
                    f"<div style='color:#93c5fd;font-size:13px;margin-bottom:6px'>• {xe(tip)}</div>",
                    unsafe_allow_html=True,
                )
