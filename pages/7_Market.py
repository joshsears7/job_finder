...
from job_market import BLS_PROJECTIONS, get_job_news, get_macro_snapshot, get_sector_changes
from utils import alert, inject_css, xe


def _safe_url(url: str) -> str:
    """Return url only if it is http(s); empty string otherwise."""
    s = str(url or "")
...
with tab_macro:
    ...
    if has_fred:
        if st.button(
            "Load Live FRED Data", type="primary", key="load_fred"
        ) or st.session_state.get("fred_data"):
            ...
            if macro:
                ...
                with c1:
                    st.markdown(
                        "<div class='section-tag'>Key Macro Indicators</div>",
                        unsafe_allow_html=True,
                    )
                with c2:
            ...
            if sectors:
                st.markdown(
                    "<div class='section-tag' style='margin-top:16px'>Sector Employment Trends</div>",
                    unsafe_allow_html=True,
                )
    ...
    st.markdown(
        "<div class='section-tag' style='margin-top:16px'>Job Market News</div>",
        unsafe_allow_html=True,
    )
        ...
        for item in news:
            ...
            if title:
                st.markdown(
                    ...
                    f"<a href='{_safe_url(link)}' target='_blank' style='color:#93c5fd;font-weight:600;"
...
with tab_bls:
    st.markdown(
        "<div class='section-tag'>BLS 2023–2033 Employment Projections</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='section-tag' style='margin-top:24px'>Hot Sectors Right Now</div>",
        unsafe_allow_html=True,
    )
...
with tab_signals:
    ...
    with hdr_c:
        st.markdown(
            "<div class='section-tag'>HackerNews & GitHub Hiring Intelligence</div>",
            unsafe_allow_html=True,
        )
        st.caption(
            "Live signals from HackerNews 'Who is Hiring?' and GitHub trending repos. Cached 4 hours."
        )
    with btn_c:
    ...
    if "market_intel" not in st.session_state:
        with st.spinner(
            "Loading hiring signals — fetching HackerNews & GitHub (may take 20–40 s)…"
        ):
            try:
    ...
    if not intel or not any(v for v in intel.values()):
            ...
            if not gh_repos:
                ...
                for repo in gh_repos[:8]:
                    ...
                    st.markdown(
                        ...
                        f"<a href='{_safe_url(url)}' target='_blank' style='color:#93c5fd;font-size:12.5px;"
        ...
        if jobicy_skills:
            st.markdown("---")
            st.markdown(
                "<div class='section-tag'>Live Job Posting Skill Demand (Jobicy)</div>",
                unsafe_allow_html=True,
            )
