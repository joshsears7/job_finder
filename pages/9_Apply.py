...
import tracker
from ai_tools import ats_scan, generate_cover_letter, generate_interview_questions
from claude_ai import stream_cover_letter_claude, stream_thankyou_claude
from utils import alert, chip, inject_css, score_ring, xe

inject_css()

st.markdown(
    "<div class='page-title'>Apply Engine</div><div class='page-sub'>One-click Apply Package: ATS scan · cover letter · targeted bullets · thank-you email — all tailored to the specific job.</div>",
...
if not profile:
    st.markdown(
        alert(
            "Upload your resume on the Dashboard first — the Apply Package is fully tailored to your profile.",
            "blue",
        ),
        unsafe_allow_html=True,
    )
    ...
    job_options.append(
        {
            ...
            "description": "",
            "source": a.get("source", ""),
            "_label": f"[Saved] {a['title']} @ {a['company']}",
        }
    )
...
if prefill:
    job_options.insert(
        0, {**prefill, "_label": f"[Selected] {prefill['title']} @ {prefill['company']}"}
    )
...
with st.expander("⚙️ Manual override / paste job description", expanded=not bool(job_options)):
    ...
    manual_title = c1.text_input(
        "Job title", value=selected_job["title"] if selected_job else "", key="apply_manual_title"
    )
    manual_company = c2.text_input(
        "Company", value=selected_job["company"] if selected_job else "", key="apply_manual_company"
    )
    manual_jd = st.text_area(
        "Paste job description",
        value=selected_job.get("description", "") if selected_job else "",
        height=140,
        key="apply_jd_paste",
    )
...
st.markdown("---")
if st.button(
    "Generate Full Apply Package", type="primary", use_container_width=True, key="gen_pkg"
):
            ...
            try:
                ...
                for kw in ats.get("missing_hard", [])[:5]:
                    tmpl = next(
                        (
                            b["suggested_bullet"]
                            for b in ats.get("suggested_bullets", [])
                            if b["keyword"] == kw
                        ),
                        None,
                    )
...
if pkg_ats:
    ...
    with st.expander(f"ATS Scan — Score: {pkg_ats['score']}%", expanded=True):
        verdict_color = {"strong": "#10b981", "medium": "#f59e0b", "weak": "#ef4444"}.get(
            pkg_ats["verdict"][0], "#64748b"
        )
        st.markdown(
        ...
        with col_found:
            st.markdown(
                "<div style='font-size:12px;font-weight:700;color:#10b981;margin-bottom:6px'>✅ Keywords Found</div>",
                unsafe_allow_html=True,
            )
            if pkg_ats["found_keywords"]:
                st.markdown(
                    " ".join(chip(k, "green") for k in pkg_ats["found_keywords"][:16]),
                    unsafe_allow_html=True,
                )
            else:
        ...
        with col_miss:
            st.markdown(
                "<div style='font-size:12px;font-weight:700;color:#ef4444;margin-bottom:6px'>❌ Missing Keywords</div>",
                unsafe_allow_html=True,
            )
            if pkg_ats["missing_hard"]:
                st.markdown(
                    " ".join(chip(k, "red") for k in pkg_ats["missing_hard"][:12]),
                    unsafe_allow_html=True,
                )
            if pkg_ats.get("missing_soft"):
                st.markdown(
                    " ".join(chip(k, "amber") for k in pkg_ats["missing_soft"][:6]),
                    unsafe_allow_html=True,
                )
        ...
        if pkg_ats.get("cliches"):
            st.markdown(
                alert(
                    f"⚠ Clichés detected: {', '.join(pkg_ats['cliches'][:4])} — remove these from your resume.",
                    "amber",
                ),
                unsafe_allow_html=True,
            )
        ...
        with st.expander("ATS Formatting Tips"):
            for tip in pkg_ats["formatting_tips"]:
                st.markdown(
                    f"<div style='font-size:12.5px;color:#64748b;margin-bottom:5px'>• {xe(tip)}</div>",
                    unsafe_allow_html=True,
                )
    ...
    with st.expander("Cover Letter", expanded=True):
        ...
        with cl_col2:
            if st.button(
                "⚡ Rewrite with Sonnet",
                key="regen_cl",
                help="Stream a higher-quality Sonnet rewrite",
            ):
    ...
    if pkg_bullets:
        with st.expander(
            f"Targeted Resume Bullets ({len(pkg_bullets)} missing keywords)", expanded=False
        ):
            st.caption(
                "Add these bullets to your resume to close the ATS keyword gap. Fill in the [brackets] with your real numbers."
            )
    ...
    with st.expander("Thank-You Note (post-interview)", expanded=False):
        st.caption(
            "Fill in the details below and Claude writes a fully personalized note — no brackets."
        )
        ty_c1, ty_c2 = st.columns(2)
        ty_interviewer = ty_c1.text_input(
            "Interviewer name",
            placeholder="e.g. Sarah Chen",
            value=st.session_state.get("pkg_ty_interviewer", ""),
            key="ty_name",
        )
        ty_topics = ty_c2.text_input(
            "What did you discuss?",
            placeholder="e.g. Python ML pipeline, team structure, growth path",
            value=st.session_state.get("pkg_ty_topics", ""),
            key="ty_topics",
        )
        ...
        edited_ty = ""
        if pkg_ty:
            edited_ty = st.text_area(
                "Edit before sending", value=pkg_ty, height=280, key="pkg_ty_edit"
            )
        else:
    ...
    try:
        ...
        with col_pdf:
            ...
            st.download_button(
                "⬇ Download Full Apply Package PDF",
                data=pdf_bytes_full,
    ...
    if _app_id_str.startswith("saved_"):
        try:
            ...
            if _current_status in (None, "saved"):
                if st.button("Mark as Applied", key="mark_applied"):
                    ...
                    st.success(
                        "Marked as applied. Follow-up reminder scheduled for 7 days from now."
                    )
                    st.rerun()
        except Exception:
            pass
        ...
        if st.button("Save & Mark as Applied", key="save_mark_applied"):
            ...
            _saved_app = next(
                (
                    a
                    for a in tracker.get_all(st.session_state.get("active_user_id", 1))
                    if a["job_id"] == _app_id_str
                ),
                None,
            )
            if _saved_app:
            ...
            st.success(
                "Saved and marked as applied. Follow-up reminder scheduled for 7 days from now."
            )
            st.rerun()
