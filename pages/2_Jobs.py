...
import tracker
from job_fetcher import CITY_PRESETS, fetch_jobs, fetch_jobs_multicity, get_fetch_warnings
from salary_intel import estimate as _bls_estimate
from salary_intel import match_role as _bls_match
from scorer import batch_score_jobs, ghost_score
from utils import alert, chip, inject_css, score_color, xe

inject_css()

st.markdown(
    "<div class='page-title'>Find Jobs</div><div class='page-sub'>Search live job postings, score them against your resume, and build your application package.</div>",
...
if not profile:
    st.markdown(
        alert("Load your resume on the Dashboard to see fit scores.", "blue"),
        unsafe_allow_html=True,
    )
...
def _title_gap(job_title: str, resume_lower: str) -> str:
    ...
    words = [
        w.lower() for w in re.findall(r"[a-zA-Z]{5,}", job_title) if w.lower() not in _STOP_WORDS
    ]
    if not words:
        return ""
...
def _generate_targeted_bullets(profile, job, missing_kw):
    ...
    for i, kw in enumerate(missing_kw[:3]):
        kw_l = kw.lower()
        tmpl = next(
            (t for k, t in templates.items() if k in kw_l or kw_l in k),
    ...
    if not lines:
        lines = [
            "• No specific missing keywords identified — your resume already aligns well with this role."
        ]
    return "\n\n".join(lines)
...
with st.expander(
    "🔍 Semantic Search — search indexed jobs by meaning, not keywords", expanded=False
):
    st.caption(
        "Search jobs already indexed from your recent searches. Type what you're looking for in plain English."
    )
    ...
    if st.button("Search Semantic Index", type="primary", key="sem_search"):
        if sem_query.strip():
            try:
                from vector_store import search_jobs as _vs_search
                from vector_store import store_stats as _vs_stats

                _stats = _vs_stats()
                if _stats["jobs_indexed"] == 0:
                    st.warning(
                        "No jobs indexed yet — run a regular search first to populate the index."
                    )
                else:
                    ...
                    if _sem_results:
                        st.markdown(
                            f"<div style='font-size:12px;color:#64748b;margin-bottom:8px'>{len(_sem_results)} semantic matches</div>",
                            unsafe_allow_html=True,
                        )
                        ...
                        st.info(
                            "No semantic matches found. Try different wording or run a regular search first."
                        )
...
# ── Search Form ────────────────────────────────────────────────────
mode = st.radio(
    "Search mode", ["City Presets", "Custom"], horizontal=True, label_visibility="collapsed"
)

with st.form("search_form"):
    col1, col2 = st.columns([3, 1])
    role = col1.text_input(
        "Role / Keywords", placeholder="e.g. Business Analyst, Marketing Coordinator"
    )
    num = col2.selectbox("Per city", [5, 10, 20], index=1)

    if mode == "City Presets":
        domestic = [
            k for k in CITY_PRESETS if CITY_PRESETS[k]["country"] == "us" or k == "🌍 Remote"
        ]
        intl = [k for k in CITY_PRESETS if CITY_PRESETS[k]["country"] != "us" and k != "🌍 Remote"]
        selected_cities = []

        st.markdown(
            "<div style='font-weight:600;margin:8px 0 4px'>United States</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            "<div style='font-weight:600;margin:8px 0 4px'>International</div>",
            unsafe_allow_html=True,
        )
...
if "jobs" in st.session_state and st.session_state.jobs:
    ...
    col_hdr.markdown(
        f"<div style='color:#64748b;font-size:13px;padding-top:8px'><b>{len(jobs)}</b> results for <b>{xe(role_label)}</b></div>",
        unsafe_allow_html=True,
    )
    sort_by = col_sort.selectbox(
        "Sort", ["Fit Score", "Company", "City", "Date"], label_visibility="collapsed"
    )
    ...
    if sort_by == "Fit Score":
        ...
        jobs = sorted(jobs, key=lambda j: j.get("city_label", j.get("location", "")))
    elif sort_by == "Date":
        jobs = sorted(jobs, key=lambda j: j.get("date", ""), reverse=True)
    ...
    min_sal_label = fc3.selectbox(
        "Min salary (listed only)",
        ["Any", "$50K+", "$75K+", "$100K+", "$125K+"],
        key="filter_salary",
    )
    hide_no_sal = fc4.checkbox(
        "Only show jobs with salary listed", value=False, key="filter_has_sal"
    )
    _MIN_SAL_MAP = {
        "Any": 0,
        "$50K+": 50_000,
    ...
    if _filter_active:
        _sal_note = (
            f" · {_no_sal_count} jobs have no salary listed"
            if not hide_no_sal and _no_sal_count > 0
            else ""
        )
        st.markdown(
    ...
    if jobs:
        ...
        with bs_col:
            if st.button("Save All Visible", key="batch_save_btn", use_container_width=True):
                ...
                saved_count = sum(
                    1 for j in jobs if tracker.save_job(j, j.get("score", 0), resume_version=_rv)
                )
    ...
    for j in jobs:
        ...
        if len(matched_all) > 3:
            matched_c += chip(f"+{len(matched_all) - 3}", "gray")
        missing_c = " ".join(chip(sk, "red") for sk in missing_all[:2])
        if len(missing_all) > 2:
            missing_c += chip(f"+{len(missing_all) - 2} missing", "amber")
        ...
        if _g_score is not None and _g_score >= 30:
            _ghost_bg = "#2d0a0a" if _g_score >= 60 else "#1a1100"
            _ghost_clr = "#fca5a5" if _g_score >= 60 else "#fbbf24"
        ...
        _date_suffix = (
            " · " + _timing_html
            if _timing_html
            else (" · <span style='color:#64748b'>" + _jdate + "</span>" if _jdate else "")
        )
        st.markdown(
            f"<div class='job-row'>"
            ...
            f"{_date_suffix}{_trans_html}{_easy_html}</div>"
        ...
        if profile and (matched_all or missing_all) and j.get("description", "").strip():
            with st.expander("📊 Why this score?", expanded=False):
                ...
                with _ws1:
                    if matched_all:
                        st.markdown(
                            "<div style='font-size:11px;font-weight:700;color:#10b981;margin-bottom:4px'>✅ Keywords you have</div>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            " ".join(chip(k, "green") for k in matched_all[:8]),
                            unsafe_allow_html=True,
                        )
                with _ws2:
                    if missing_all:
                        st.markdown(
                            "<div style='font-size:11px;font-weight:700;color:#ef4444;margin-bottom:4px'>❌ Keywords you're missing</div>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            " ".join(chip(k, "red") for k in missing_all[:6]),
                            unsafe_allow_html=True,
                        )
                        if len(missing_all) <= 3:
                            st.caption(
                                f"Only {len(missing_all)} gaps — add these to your resume/LinkedIn to reach 90%+"
                            )
        ca, cb, cc, cd = st.columns([1.3, 1.5, 2.2, 1.5])
        with ca:
            if tracker.is_saved(j["id"]):
                st.button("✅ Saved", key=f"sv_{j['id']}", disabled=True, use_container_width=True)
            else:
                if st.button("Save", key=f"sv_{j['id']}", use_container_width=True):
                    tracker.save_job(
                        j, s, resume_version=st.session_state.get("active_resume_version")
                    )
                    st.rerun()
        with cb:
            _jurl2 = str(j.get("url", "")).strip()
            if _jurl2.startswith(("https://", "http://")):
        ...
        with cc:
            ...
            if st.button(
                ...
                type="primary" if is_active_pkg else "secondary",
            ):
                if is_active_pkg:
                    for k in [
                        "apply_pkg_id",
        ...
        with cd:
            if st.button("🏢 Research", key=f"rc_{j['id']}", use_container_width=True):
                st.session_state[f"show_co_{j['id']}"] = not st.session_state.get(
                    f"show_co_{j['id']}", False
                )
        ...
        with btn_row2[2]:
            if st.button("🔗 Similar Jobs", key=f"sim_{j['id']}", use_container_width=True):
                st.session_state[f"show_sim_{j['id']}"] = not st.session_state.get(
                    f"show_sim_{j['id']}", False
                )
        ...
        if st.session_state.get(f"show_sim_{j['id']}"):
            try:
                ...
                if _sims:
                    st.markdown(
                        "<div style='font-size:11px;font-weight:700;color:#64748b;margin:6px 0 4px;text-transform:uppercase'>Similar jobs from index</div>",
                        unsafe_allow_html=True,
                    )
                    for _sim in _sims:
        ...
        if fit_result:
            st.markdown(
                ...
                f"<div style='font-size:11px;font-weight:700;color:#7c3aed;margin-bottom:6px'>🎯 YOUR HONEST FIT — {xe(j['title'])} @ {xe(j['company'])}</div>"
        ...
        if st.session_state.get(f"ip_{j['id']}"):
            ...
            st.markdown(
                ...
                f"letter-spacing:.06em;margin-bottom:10px'>🎓 Interview Prep Pack — {xe(j['title'])} @ {xe(j['company'])}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
            ip_q, ip_star, ip_ty = st.tabs(
                ["Likely Questions", "STAR Framework", "Tell Me About Yourself"]
            )
            with ip_q:
            ...
            with ip_star:
                st.caption(
                    "Use the STAR method for every behavioural question — interviewers score you on this."
                )
                for step in ip["star"]:
                    st.markdown(
                        f"<div class='card-slate' style='margin-bottom:8px;font-size:13px'>{step}</div>",
                        unsafe_allow_html=True,
                    )
                st.markdown(
                    alert(
                        "💡 Prepare 3–5 STAR stories before any interview. Most questions are variations of the same 8 themes.",
                        "blue",
                    ),
                    unsafe_allow_html=True,
                )
        ...
        if st.session_state.get(f"show_co_{j['id']}"):
                ...
                with st.spinner(f"Looking up {co_name}…"):
                    try:
                        ...
                        r2 = _req.get(
                            ...
                            timeout=5,
                            headers={"User-Agent": "CareerIQ/1.0"},
                        )
            ...
            st.markdown(
                ...
                f"<div style='font-weight:700;font-size:14px;margin-bottom:6px'>🏢 {xe(co_name)}</div>"
                + (
                    ...
                    if co_data
                    else "<div style='font-size:12px;color:#94a3b8'>No Wikipedia entry found for this company.</div>"
                )
                ...
                + (
                    f"<a href='{co_data['wiki_url']}' target='_blank' style='color:#2563eb'>Wikipedia ↗</a>"
                    if co_data and co_data.get("wiki_url")
                    else ""
                )
                + "</div></div>",
                unsafe_allow_html=True,
                ...
                with ci1:
                    if ci_data.get("culture_signals"):
                        ...
                        for s in ci_data["culture_signals"]:
                            st.markdown(
                                f"<div style='font-size:12px;padding:4px 0;border-bottom:1px solid #1e293b;color:#cbd5e1'>• {s}</div>",
                                unsafe_allow_html=True,
                            )
                    if ci_data.get("red_flags"):
                        ...
                        for rf in ci_data["red_flags"]:
                            st.markdown(
                                f"<div style='font-size:12px;padding:4px 0;color:#dc2626'>• {rf}</div>",
                                unsafe_allow_html=True,
                            )
                with ci2:
                    if ci_data.get("day_in_life"):
                        ...
                        for d in ci_data["day_in_life"]:
                            st.markdown(
                                f"<div style='font-size:12px;padding:4px 0;border-bottom:1px solid #1e293b;color:#cbd5e1'>• {d}</div>",
                                unsafe_allow_html=True,
                            )
                    if ci_data.get("likely_questions"):
                        ...
                        for q in ci_data["likely_questions"]:
                            st.markdown(
                                f"<div style='font-size:12px;padding:4px 0;color:#2563eb'>• {q}</div>",
                                unsafe_allow_html=True,
                            )
        ...
        if _matches:
            ...
            st.markdown(
                ...
                f"🤝 <b>Referral Radar:</b> You know someone at <b>{xe(j['company'])}</b> — {xe(_names)}"
                f"</div>",
                unsafe_allow_html=True,
            )
            if st.button("Draft Referral Ask", key=f"ref_{j['id']}", use_container_width=False):
                st.session_state[f"ref_msg_{j['id']}"] = _draft_referral_ask(
                    _matches[0], j, profile
                )
    ...
    st.markdown("---")
    with st.expander(
        "📊 Market Intelligence — BLS outlook, hiring signals & job market news", expanded=False
    ):
        ...
        with _mi_tab1:
            try:
                ...
                with _mc1:
                    st.markdown(
                        "<div style='font-size:12px;font-weight:700;color:#10b981;margin-bottom:8px'>Fastest Growing Roles</div>",
                        unsafe_allow_html=True,
                    )
                ...
                with _mc2:
                    st.markdown(
                        "<div style='font-size:12px;font-weight:700;color:#ef4444;margin-bottom:8px'>Fastest Declining Roles</div>",
                        unsafe_allow_html=True,
                    )
        ...
        with _mi_tab2:
            if st.button(
                "Load HackerNews + GitHub Signals", key="jobs_load_signals"
            ) or st.session_state.get("market_intel"):
                ...
                with _sig_c1:
                    st.markdown(
                        "<div style='font-size:12px;font-weight:700;color:#f59e0b;margin-bottom:8px'>HackerNews — Most-wanted skills</div>",
                        unsafe_allow_html=True,
                    )
                    for _term, _cnt in (_intel.get("hn_skills") or [])[:12]:
                        _bpct = min(
                            ...
                            int(
                                _cnt
                                / max(
                                    (_intel["hn_skills"][0][1] if _intel.get("hn_skills") else 1), 1
                                )
                                * 100
                            ),
                        )
                ...
                with _sig_c2:
                    st.markdown(
                        "<div style='font-size:12px;font-weight:700;color:#3b82f6;margin-bottom:8px'>GitHub — Trending repos</div>",
                        unsafe_allow_html=True,
                    )
        ...
        with _mi_tab3:
            if st.button("Load Job Market News", key="jobs_load_news") or st.session_state.get(
                "job_news"
            ):
                ...
                for _item in st.session_state.get("job_news") or []:
                    if _item.get("title"):
                        _nlink = _item.get("link", "")
                        _safe_nlink = (
                            _nlink if str(_nlink).startswith(("https://", "http://")) else ""
                        )
                        st.markdown(
                            f"<div style='padding:8px 0;border-bottom:1px solid #1e293b'>"
                            f"<a href='{_safe_nlink}' target='_blank' style='color:#93c5fd;font-weight:600;"
                            f"font-size:13px;text-decoration:none'>{xe(_item['title'])}</a>"
                            f"<div style='font-size:11px;color:#475569;margin-top:2px'>"
                            f"{xe(_item.get('source', ''))} · {xe(_item.get('date', ''))}</div>"
                            f"</div>",
                            unsafe_allow_html=True,
                        )
            else:
    ...
    if pkg_job:
        from ai_tools import ats_scan as _ats_scan
        from ai_tools import generate_cover_letter
        ...
        if not profile:
            st.markdown(
                alert(
                    "Load your resume on the Dashboard to generate personalized content.", "blue"
                ),
                unsafe_allow_html=True,
            )
        else:
            if (
                "pkg_ats" not in st.session_state
                or st.session_state.get("pkg_ats_id") != pkg_job["id"]
            ):
                if pkg_job.get("description", "").strip():
                    with st.spinner("Running ATS scan…"):
                        st.session_state.pkg_ats = _ats_scan(
                            profile["raw_text"], pkg_job["description"]
                        )
            ...
            st.markdown(
                "<div class='section-tag' style='margin-top:8px'>ATS Compatibility</div>",
                unsafe_allow_html=True,
            )
            if ats:
                _vc_map = {"strong": "#059669", "medium": "#d97706", "weak": "#dc2626"}
                vc_color = _vc_map.get(ats.get("verdict", ["weak"])[0], "#dc2626")
                a1, a2, a3, a4 = st.columns([1, 1, 1, 3])
                a1.markdown(
                    f"<div style='text-align:center'><div style='font-size:2.2rem;font-weight:900;color:{vc_color}'>{ats['score']}%</div><div style='font-size:11px;color:#94a3b8'>ATS Score</div></div>",
                    unsafe_allow_html=True,
                )
                ...
                with a4:
                    ...
                    if _mh:
                        st.markdown(
                            "<span style='font-size:12px;font-weight:600;color:#dc2626'>Hard skills missing: </span>"
                            + " ".join(chip(k, "red") for k in _mh[:5]),
                            unsafe_allow_html=True,
                        )
                    if _ms:
                        st.markdown(
                            "<span style='font-size:12px;font-weight:600;color:#d97706'>Soft skills to highlight: </span>"
                            + " ".join(chip(k, "amber") for k in _ms[:4]),
                            unsafe_allow_html=True,
                        )
                    st.markdown(
                        alert(
                            ats["verdict"][1],
                            "green"
                            if ats["verdict"][0] == "strong"
                            else "amber"
                            if ats["verdict"][0] == "medium"
                            else "red",
                        ),
                        unsafe_allow_html=True,
                    )
                ...
                st.markdown(
                    alert(
                        "Paste the job description in the search results to get ATS score.", "amber"
                    ),
                    unsafe_allow_html=True,
                )
            ...
            with bc:
                st.markdown(
                    "<div class='section-tag'>Targeted Resume Bullets</div>", unsafe_allow_html=True
                )
                ...
                if st.button("Generate Bullets", key="pkg_bullets_btn", use_container_width=True):
                    missing_kw = (
                        ats["missing_keywords"][:3]
                        if ats and ats.get("missing_keywords")
                        else ["data analysis", "project management", "stakeholder communication"]
                    )
                    st.session_state.pkg_bullets = _generate_targeted_bullets(
                        profile, pkg_job, missing_kw
                    )
                if st.session_state.get("pkg_bullets"):
                    st.text_area(
                        "Add to your resume",
                        value=st.session_state.pkg_bullets,
                        height=200,
                        key="pkg_b_edit",
                        label_visibility="collapsed",
                    )
            ...
            with tc:
                st.markdown(
                    "<div class='section-tag'>Thank-You Email</div>", unsafe_allow_html=True
                )
                st.caption("Send within 24 hours of your interview")
                if st.button(
                    "Generate Thank-You Email", key="pkg_ty_btn", use_container_width=True
                ):
                    with st.spinner("Writing…"):
            ...
            with pkg_tip_col:
                st.markdown(
                    alert(
                        "💡 Generate the sections above, then download your full application package as one PDF.",
                        "blue",
                    ),
                    unsafe_allow_html=True,
                )
