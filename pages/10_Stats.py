...
st.markdown(
    "<div class='page-title'>Platform Stats</div><div class='page-sub'>Real usage — tracked automatically, no inflation.</div>",
...
with tab_usage:
    ...
    if not stats or not any(v for v in stats.values() if v is not None):
    ...
    try:
        ...
        st.caption(
            "Jobs are indexed automatically after each search. Resumes are indexed on upload."
        )
...
with tab_scanner:
    st.markdown(
        "<div class='section-tag'>Background Scanner Run History</div>", unsafe_allow_html=True
    )
    ...
    if not runs:
        ...
        try:
            ...
            df = df.rename(
                columns={
                    ...
                    "jobs_saved": "Auto-Saved",
                    "jobs_notified": "Alerts Sent",
                }
            )
        ...
        # Run table
        st.markdown(
            "<div class='section-tag' style='margin-top:16px'>Run Log</div>", unsafe_allow_html=True
        )
        for r in runs[:20]:
...
with tab_pipeline:
    ...
    if not apps:
        st.markdown(
            "<div style='color:#64748b;font-size:13px'>No applications tracked yet.</div>",
            unsafe_allow_html=True,
        )
    else:
        ...
        if scores:
            ...
            st.markdown(
                f"<div style='margin-top:12px;font-size:13px;color:#64748b'>Average fit score across saved applications: <b style='color:#3b82f6'>{avg_score}%</b></div>",
                unsafe_allow_html=True,
            )

        # Resume version A/B stats (legacy tracker-based)
        vstats = tracker.get_version_stats(user_id)
        if vstats:
            st.divider()
            st.markdown(
                "<div class='section-tag'>Resume Version A/B Performance</div>",
                unsafe_allow_html=True,
            )
            for vname, vs in vstats.items():
                cols = st.columns([2, 1, 1, 1, 1])
                cols[0].markdown(
                    f"<div style='font-size:13px;font-weight:600;color:#f1f5f9;padding-top:4px'>{xe(vname)}</div>",
                    unsafe_allow_html=True,
                )
...
with tab_ab:
    ...
    try:
        ...
        if not ab_stats:
            st.markdown(
                alert("No resume versions tracked yet. Add a version below to start.", "blue"),
                unsafe_allow_html=True,
            )
        else:
            ...
            _header_cols[0].markdown(
                "<div style='font-size:11px;font-weight:700;color:#64748b'>VERSION</div>",
                unsafe_allow_html=True,
            )
            for _lbl, _ci in [
                ("APPS", 1),
                ...
                _header_cols[_ci].markdown(
                    f"<div style='font-size:11px;font-weight:700;color:#64748b'>{_lbl}</div>",
                    unsafe_allow_html=True,
                )
            ...
            for vs_row in ab_stats:
                ...
                row_bg = (
                    "border-left:3px solid #10b981" if is_best else "border-left:3px solid #1e293b"
                )
                ...
                cols[5].markdown(
                    f"<div style='font-size:18px;font-weight:900;color:{rr_color};padding-top:4px'>{rr}%</div>",
                    unsafe_allow_html=True,
                )
                cols[6].markdown(
                    f"<div style='font-size:14px;color:#64748b;padding-top:6px'>{vs_row.get('avg_days') or '—'}</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    "<div style='border-bottom:1px solid #1e293b;margin:4px 0'></div>",
                    unsafe_allow_html=True,
                )
        ...
        with st.expander("➕ Track a new resume version"):
            ab_vname = st.text_input(
                "Version name", placeholder="e.g. Version A — Skills focus", key="ab_vname"
            )
            ab_vlabel = st.text_input(
                "Short label (optional)", placeholder="e.g. No objective", key="ab_vlabel"
            )
            ab_vtext = st.text_area(
                "Paste resume text (for comparison)", height=120, key="ab_vtext"
            )
        ...
        with st.expander("📝 Log an application"):
            if ab_stats:
                ...
                chosen_ver = st.selectbox(
                    "Resume version used", list(ver_opts.keys()), key="ab_log_ver"
                )
...
with tab_eval:
    st.markdown(
        "<div class='section-tag'>AI Output Quality Dashboard</div>", unsafe_allow_html=True
    )
    st.caption(
    ...
    try:
        from eval_engine import get_eval_history, get_eval_summary
        ...
        if summary.get("total", 0) == 0:
            st.markdown(
                alert(
                    "No evaluations yet — generate a cover letter, LinkedIn About, or interview coaching to populate this dashboard.",
                    "blue",
                ),
                unsafe_allow_html=True,
            )
            ...
            if by_type:
                st.markdown(
                    "<div class='section-tag' style='margin-top:16px'>Quality by Output Type</div>",
                    unsafe_allow_html=True,
                )
            ...
            if history:
                st.markdown(
                    "<div class='section-tag' style='margin-top:16px'>Recent Evaluations</div>",
                    unsafe_allow_html=True,
                )
